...
MODULE_DESCRIPTION("Example for threads in a LKM");
 ...
 */
static ssize_t personal_write(struct file *File, const char *user_buffer, size_t count, loff_t *offs) {
 ...
 */
static int __init personal_module_init(void) {
	int t1 = 1, t2 = 2;
	...
	if(register_chrdev(PERSONAL_MAJOR, "dev_nr", &fops) != 0) {
	...
	if(kthread_1 != NULL){
		...
		unregister_chrdev(PERSONAL_MAJOR, "dev_nr");
		return -1;
	}
 ...
 */
static void __exit personal_module_exit(void) {
...
module_init(personal_module_init);
module_exit(personal_module_exit);
